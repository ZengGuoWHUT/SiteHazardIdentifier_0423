...
namespace RevitVoxelzation
{
    ...
    public class CellIndex3D : IEquatable<CellIndex3D>
    {
        ...
        public static CellIndex3D operator +(CellIndex3D a,CellIndex3D b)
        {
            return new CellIndex3D(a.Col + b.Col, a.Row + b.Row, a.Layer + b.Layer);
        }
        ...
        {
            return Math.Sqrt(Math.Pow(this.Col, 2) + Math.Pow(this.Row, 2) + Math.Pow(this.Layer, 2));
        }
            ...
            for(int col=0;col<=voxInSld.GetUpperBound(0);col++)
            {
                for(int row=0;row<=voxInSld.GetUpperBound(1);row++)
                {
                    ...
                    if(voxData !=null && voxData.Count > 0)
                    {
                        ...
                        {
                            if(double.IsNaN( vd.BottomElevation))
                            {
    ...
    public class MeshTriangle 
    {
        ...
        public IEnumerable<GridPoint> ObtainGridPoints(Vec3 origin, double voxelSize, GridPoint[] solidVerticesGPs)
        {
            ...
            Vec3 v1 = this.Host.Vertices[vi1];
            Vec3 v2 = this.Host.Vertices[vi2];
            //20260403:TODO:Check triangle validity
            //get triangle boundary
            //compute triNorm
            var vec01 = v1 - v0;
            var vec12 = v2 - v1;
            var triNorm = vec01.CrossProduct(vec12);
            if(triNorm.GetSquareLen()>1e-9)//other wise, the triangle is a silver one, do not generate voxels
            {
                var norm = triNorm.Normalize();
    ...
    {
        public BVHTreeNode RootNode { get; set; }

        public void Build(List<LightWeightVoxelElement> elems,int boxLimit,int voxelSize)
        {
            //expand voxBox
            List<VoxelBox> boxes = new List<VoxelBox>();
            ...
            {
                box.CalculateCenter(voxelSize);
            }
            //create a root node
            var root = new BVHTreeNode();
            this.RootNode = root;
            GenerateNode(root, boxes, boxLimit,voxelSize);
        }

        private void GenerateNode(BVHTreeNode thisNode, List<VoxelBox> boxes2Add,int boxLimit,int voxSize)
        {
            //get the center of the node
            thisNode.Bound = new BVHNodeBoundary(boxes2Add, voxSize);
            ...
            else
            {
                ...
                {
                    boxes2Add.Sort((a, b) =>a.CenterMM.Col.CompareTo( b.CenterMM.Col));
                }
                else if(rowSize>=colSize && rowSize>=layerSize)
                {
                    boxes2Add.Sort((a, b) => a.CenterMM.Row.CompareTo(b.CenterMM.Row));
                }
                else
                {
                    boxes2Add.Sort((a, b) => a.CenterMM.Layer.CompareTo(b.CenterMM.Layer));
                }
                //divide voxels
                ...
                List<VoxelBox> boxesL = new List<VoxelBox>();
                List<VoxelBox> boxesR = new List<VoxelBox>();
                ...
                GenerateNode(leftNode, boxesL, boxLimit,voxSize);
                GenerateNode(rightNode, boxesR, boxLimit,voxSize);
            }
        }

        public IEnumerable<BVHTreeNode> GetAllNodes()
        {
            Stack<BVHTreeNode> stkNodes = new Stack<BVHTreeNode>();
    ...
    public class BVHTreeNode
    {
        ...
        public BVHTreeNode Parent { get; set; }
        public List<VoxelBox> VoxelBoxes { get; set; }
        public BVHNodeBoundary Bound { get; set; }
        
    }

    public class BVHNodeBoundary
    {
        public CellIndex3D Min { get; set; }
        public CellIndex3D Max { get; set; }
        public BVHNodeBoundary(List<VoxelBox> boxes,int voxelSize)
        {
            //get voxel boundary
            int colMin = int.MaxValue;
            int rowMin = int.MaxValue;
            ...
            foreach (var vox in boxes)
            {
                colMin = Math.Min(vox.Min.Col, colMin);
                colMax = Math.Max(vox.Max.Col+1, colMax);
                rowMin = Math.Min(vox.Min.Row, rowMin);
                rowMax = Math.Max(vox.Max.Row+1, rowMax);
                layerMin = Math.Min(layerMin, vox.Min.Layer);
                layerMax = Math.Max(layerMax, vox.Max.Layer);
            }
            if(layerMin>100000)
            {

            }
            this.Max =new CellIndex3D(colMax*voxelSize, rowMax* voxelSize, layerMax);
            this.Min =new CellIndex3D(colMin*voxelSize ,rowMin*voxelSize ,layerMin)  ;
        }
    }
